...
def parse_arguments():
    ...
    parser.add_argument('--train_dataset', type=str, default='none', choices=['none', 'advbench', 'harmbench'], help='Dataset to use for training')
    return parser.parse_args()

def load_and_sample_datasets(cfg, train_dataset="none"):
    """
    ...
    elif train_dataset == "advbench":
        harmful_train = random.sample(load_dataset_split(harmtype='harmful_advbench', split='train', instructions_only=True), cfg.n_train)
    elif train_dataset == "harmbench":
        harmful_train = random.sample(load_dataset_split(harmtype='harmful_harmbench', split='train', instructions_only=True), cfg.n_train)
...
def generate_and_save_candidate_directions(cfg, model_base, harmful_train, harmless_train, train_dataset="none"):
    ...
    if train_dataset == "none":
        ...
        torch.save(mean_diffs, os.path.join(cfg.artifact_path(), 'generate_directions/advbench_mean_diffs.pt'))
    elif train_dataset == "harmbench":
        torch.save(mean_diffs, os.path.join(cfg.artifact_path(), 'generate_directions/harmbench_mean_diffs.pt'))
    return mean_diffs
...
def run_pipeline(model_path, train_dataset):
    ...
    harmful_train, harmless_train, harmful_val, harmless_val = load_and_sample_datasets(cfg, train_dataset)
    
    # # Filter datasets based on refusal scores
    # harmful_train, harmless_train, harmful_val, harmless_val = filter_data(cfg, model_base, harmful_train, harmless_train, harmful_val, harmless_val)

    # 1. Generate candidate refusal directions
    candidate_directions = generate_and_save_candidate_directions(cfg, model_base, harmful_train, harmless_train, train_dataset)
...
if __name__ == "__main__":
    ...
    run_pipeline(model_path=args.model_path, train_dataset=args.train_dataset)
