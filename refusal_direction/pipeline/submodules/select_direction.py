...
def filter_fn(refusal_score, steering_score, kl_div_score, layer, n_layer, kl_threshold=None, induce_refusal_threshold=None, prune_layer_percentage=0.20) -> bool:
    if math.isnan(refusal_score) or math.isnan(steering_score) or math.isnan(kl_div_score):
        print(f"Filtering out direction with NaN values: refusal_score={refusal_score}, steering_score={steering_score}, kl_div_score={kl_div_score}")
        return True
    if prune_layer_percentage is not None and layer >= int(n_layer * (1.0 - prune_layer_percentage)):
        print(f"Filtering out direction from layer {layer} (pruned layers: {int(n_layer * (1.0 - prune_layer_percentage))})")
        return True
    if kl_threshold is not None and kl_div_score > kl_threshold:
        print(f"Filtering out direction with KL divergence {kl_div_score} (threshold: {kl_threshold})")
        return True
    if induce_refusal_threshold is not None and steering_score < induce_refusal_threshold:
        print(f"Filtering out direction with steering score {steering_score} (threshold: {induce_refusal_threshold})")
        return True
    return False
